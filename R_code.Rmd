...
easypackages::libraries(
  ...
  "magrittr", "readr", "EnvStats", "sjPlot", "egg","ggplot2","dplyr","RVAideMemoire","car","lsmeans") 
```
...
```{r}
bloodchem.data <- read.csv2(file = "data/bloodchemistry_data.csv",sep = ";",dec = ".") %>% 
  mutate(Ring = as.character(Ring)) %>% 
  mutate(BCI = as.numeric(BCI)) %>%
...
m.c <- 
  ...
  geom_text(data = data.frame(Wing = Inf, Weight = Inf, Treatment = "control"),
            label = paste("R² =", round(models$R2_conditional[2],2)),
            hjust = 3.1, vjust = 1.5, col = "Black", size= 3) +
  geom_text(data = data.frame(Wing = 3.9, Weight = Inf, Treatment = "control"),
            label = "\U2642", hjust = 3.5, vjust = 0, col = "Black", size = 7) +
...
m.m <- 
  ...
  scale_x_log10(breaks = seq(0, 30, 5), limits = c(3.8, 30.1)) +
  geom_text(data = data.frame(Wing = Inf, Weight = Inf, Treatment = "control"),
            label = paste("R² =", round(models$R2_conditional[4],2)),
            hjust = 3.1, vjust = 1.7, col = "Black", size= 3) 
...
m.w <- 
  ...
  scale_x_log10(breaks = seq(0, 30, 5), limits = c(3.8, 30.1)) +
  geom_text(data = data.frame(Wing = Inf, Weight = Inf, Treatment = "control"),
            label = paste("R² =", round(models$R2_conditional[6],2)),
            hjust = 3.1, vjust = 1.7, col = "Black", size= 3) 
...
f.c <- 
  ...
  scale_x_log10(breaks = seq(0, 30, 5), limits = c(3.8, 30.1)) +
  geom_text(data = data.frame(Wing = Inf, Weight = Inf, Treatment = "control"),
            label = paste("R² =", round(models$R2_conditional[1],2)), 
            hjust = 3.1, vjust = 1.7, col = "Black", size= 3) +
  geom_text(data = data.frame(Wing = 3.9, Weight = Inf, Treatment = "control"),
            label = "\U2640", hjust = 3.5, vjust = -0.1, col = "Black", size = 7)+
  coord_cartesian(clip = 'off') 
...
f.m <- 
  ...
  scale_x_log10(breaks = seq(0, 30, 5), limits = c(3.8, 30.1)) +
  geom_text(data = data.frame(Wing = Inf, Weight = Inf, Treatment = "control"),
            label = paste("R² =", round(models$R2_conditional[3],2)),
            hjust = 3.1, vjust = 1.7, col = "Black", size = 3) 
...
f.w <- 
  ...
  geom_abline(intercept = models$Intercept[5], slope = models$Slope[5],
              lty = 1, size = 1.1, col = "#33A02C") +
  geom_point(size = 1.5, alpha = .6, aes(shape = Sampling), col = "#33A02C") +
  guides(fill = guide_legend(text.vjust = 0.3)) +
  theme_article() + 
  theme(
    axis.title = element_blank(),
    axis.text.y = element_blank(),
  ...
  scale_x_log10(breaks = seq(0, 30, 5), limits = c(3.8, 30.1)) +
  geom_text(data = data.frame(Wing = Inf, Weight = Inf, Treatment = "control"),
            label = paste("R² =", round(models$R2_conditional[5],2)), 
            hjust = 3.1, vjust = 1.7, col = "Black", size= 3) 
...
## -----------------------------------------------------------------------------
g1_pred_delta <- ggplot(pred.bci, aes(x = x, y = predicted, color=x, fill=x)) +
  # geom_hline(yintercept = mean(pred.bci$predicted, na.rm = T), linetype = "dashed") + 
  labs(x = "Treatment", y = "\U0394 body condition\n(z-transformed)") + 
  scale_fill_manual(values=c("#1F78B4",  "#FF7F00", "#33A02C"), labels = c("control","Malarone®","water")) +
...
g2_pred <- ggplot(mod_gr, aes(x = x, y = predicted)) +
  ...
  labs(x = "Treatment", y = "growth rate [g/day]\n(z-transformed)") + 
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), colour = 'black', size = 1.5, width = .1) + 
  geom_errorbar(aes(ymin = predicted - std.error , ymax = predicted + std.error),
                size = 8, colour = c("#1F78B4", "#33A02C", "#FF7F00"), width = 0) + 
  geom_point(color = "black", size = 2) + 
...
I <-
  ...
  theme(
    ...
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    #axis.text.x = element_text(angle=45, vjust=0.65, hjust=0.9)
  ) +
  scale_color_manual(values = c("#A6CEE3", "#1F78B4", "#FDBF6F", "#FF7F00")) +
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
...
## Leveling up graph J + K done with photoshop... - I tried with inset_element() but couldn't get a handle on it 
```
...
#### Comparing parameters at post-treatment across control, low and high dosage of drug  

```{r}

# three groupings of drug dosage: low, medium and high (under 9.68 mL/Kg, between 9.68 and 12.3 and above 12.3 mL/Kg)

bloodchem.data$dosage_grp=cut(bloodchem.data$Dosage, 3, labels = c('low','medium','high'))

newdata=bloodchem.data[bloodchem.data$Treatment %in% c('Post-Mal','Post-Control'),]
newdata$dosage_grp=as.character(newdata$dosage_grp)
newdata$dosage_grp[is.na(newdata$dosage_grp) == T] <- 'untreated'
newdata$dosage_grp=factor(newdata$dosage_grp, levels=c('low','medium','untreated'), labels=c("Low", "High", "Control"))
...
g1=ggplot(means)+aes(x = dosage_grp, y = MEAN)+
  scale_color_manual(values = c("#FDBF6F", "#FF7F00","#1F78B4"))+ 
  labs(y="AP")+
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
  geom_errorbar(aes(col = dosage_grp, ymin = MEAN - SD, ymax = MEAN + SD),
                width = 0, size = 8) +
  geom_point(size=2)+
  geom_jitter(data = newdata, aes(y = AP), height = 0, size = 2, alpha = .6, width = .2, shape = 1) +
  theme_article(base_size = 8) +
  theme(legend.position = "none",
...
g2=ggplot(means)+aes(x = dosage_grp, y = MEAN)+
  scale_color_manual(values = c("#FDBF6F", "#FF7F00","#1F78B4"))+ 
  labs(y="AST")+
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
  geom_errorbar(aes(col = dosage_grp, ymin = MEAN - SD, ymax = MEAN + SD),
                width = 0, size = 8) +
  geom_point(size=2)+
  geom_jitter(data = newdata, aes(y = AST), height = 0, size = 2, alpha = .6, width = .2, shape = 1) +
  theme_article(base_size = 8) +
  theme(legend.position = "none",
        axis.title.x = element_blank(), axis.text.x = element_blank())
...
g3=ggplot(means)+aes(x = dosage_grp, y = MEAN)+
  scale_color_manual(values = c("#FDBF6F", "#FF7F00","#1F78B4"))+ 
  labs(y="GGT")+
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
  geom_errorbar(aes(col = dosage_grp, ymin = MEAN - SD, ymax = MEAN + SD),
                width = 0, size = 8) +
  geom_point(size=2)+
  geom_jitter(data = newdata, aes(y = GGT), height = 0, size = 2, alpha = .6, width = .2, shape = 1) +
  theme_article(base_size = 8) +
  theme(legend.position = "none",
        axis.title.x = element_blank(), axis.text.x = element_blank())
...
g4=ggplot(means)+aes(x = dosage_grp, y = MEAN)+
  scale_color_manual(values = c("#FDBF6F", "#FF7F00","#1F78B4"))+ 
  labs(y="BuChE")+
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
  geom_errorbar(aes(col = dosage_grp, ymin = MEAN - SD, ymax = MEAN + SD),
                width = 0, size = 8) +
  geom_point(size=2)+
  geom_jitter(data = newdata, aes(y = BuChE), height = 0, size = 2, alpha = .6, width = .2, shape = 1) +
  theme_article(base_size = 8) +
  theme(legend.position = "none",
        axis.title.x = element_blank(), axis.text.x = element_blank())
...
g6=ggplot(means)+aes(x = dosage_grp, y = MEAN)+
  scale_color_manual(values = c("#FDBF6F", "#FF7F00","#1F78B4"))+ 
  labs(y="BA")+
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
  geom_errorbar(aes(col = dosage_grp, ymin = MEAN - SD, ymax = MEAN + SD),
                width = 0, size = 8) +
  geom_point(size=2)+
  geom_jitter(data = newdata, aes(y = BA), height = 0, size = 2, alpha = .6, width = .2, shape = 1) +
  theme_article(base_size = 8) +
  theme(legend.position = "none",
        axis.title.x = element_blank(), axis.text.x = element_blank())
...
g7=ggplot(means)+aes(x = dosage_grp, y = MEAN)+
  scale_color_manual(values = c("#FDBF6F", "#FF7F00","#1F78B4"))+ 
  labs(y="LDH")+
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
  geom_errorbar(aes(col = dosage_grp, ymin = MEAN - SD, ymax = MEAN + SD),
                width = 0, size = 8) +
  geom_point(size=2)+
  geom_jitter(data = newdata, aes(y = LDH), height = 0, size = 2, alpha = .6, width = .2, shape = 1) +
  theme_article(base_size = 8) +
  theme(legend.position = "none",
        axis.title.x = element_blank(), axis.text.x = element_blank())
...
g8=ggplot(means)+aes(x = dosage_grp, y = MEAN)+
  scale_color_manual(values = c("#FDBF6F", "#FF7F00","#1F78B4"))+ 
  labs(y="CK")+
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
  geom_errorbar(aes(col = dosage_grp, ymin = MEAN - SD, ymax = MEAN + SD),
                width = 0, size = 8) +
  geom_point(size=2)+
  geom_jitter(data = newdata, aes(y = CK), height = 0, size = 2, alpha = .6, width = .2, shape = 1) +
  theme_article(base_size = 8) +
  theme(legend.position = "none",
        axis.title.x = element_blank(), axis.text.x = element_text(angle = 30, vjust = -0.00001))
...
g9=ggplot(means)+aes(x = dosage_grp, y = MEAN)+
  scale_color_manual(values = c("#FDBF6F", "#FF7F00","#1F78B4"))+ 
  labs(y="ALB")+
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
  geom_errorbar(aes(col = dosage_grp, ymin = MEAN - SD, ymax = MEAN + SD),
                width = 0, size = 8) +
  geom_point(size=2)+
  geom_jitter(data = newdata, aes(y = ALB), height = 0, size = 2, alpha = .6, width = .2, shape = 1) +
  theme_article(base_size = 8) +
  theme(legend.position = "none",
        axis.title.x = element_blank(), axis.text.x = element_text(angle = 30, vjust = -0.00001))
...
g10=ggplot(means)+aes(x = dosage_grp, y = MEAN)+
  scale_color_manual(values = c("#FDBF6F", "#FF7F00","#1F78B4"))+ 
  labs(y="TP")+
  geom_errorbar(aes(ymin = LCL, ymax = UCL),
                width = 0.1, size = 1) +
  geom_errorbar(aes(col = dosage_grp, ymin = MEAN - SD, ymax = MEAN + SD),
                width = 0, size = 8) +
  geom_point(size=2)+
  geom_jitter(data = newdata, aes(y = TP), height = 0, size = 2, alpha = .6, width = .2, shape = 1) +
  theme_article(base_size = 8) +
  theme(legend.position = "none",
        axis.title.x = element_blank(), axis.text.x = element_text(angle = 30, vjust = -0.00001))
...
#### Testing for post-treatment difference according to drug dosage groups and controls

```{r}

# AP
pander::pandoc.table(car::Anova(lm(AP~dosage_grp, data=newdata)))

# AST
pander::pandoc.table(car::Anova(lm(AST~dosage_grp, data=newdata)))

# GGT
pander::pandoc.table(car::Anova(lm(GGT~dosage_grp, data=newdata)))

# BuChE
lm1=lm(BuChE~dosage_grp, data=newdata)
pander::pandoc.table(car::Anova(lm1))

lsm1=lsmeans(lm1,~ dosage_grp)
contrast(lsm1, 'pairwise')                      # est= -0.31, p=0.08

# BA
pander::pandoc.table(car::Anova(lm(BA~dosage_grp, data=newdata)))

# LDH
pander::pandoc.table(car::Anova(lm(LDH~dosage_grp, data=newdata)))

# CK
lm2=lm(CK~dosage_grp, data=newdata)
car::Anova(lm2)

lsm2=lsmeans(lm2, ~dosage_grp)
contrast(lsm2, "pairwise")          #  est= -1132, p=0.06 low - medium

# ALB
lm3=lm(ALB~dosage_grp, data=newdata)
car::Anova(lm3)

lsm3=lsmeans(lm3, ~dosage_grp)
contrast(lsm3, "pairwise")            # est 1.34, p= 0.1  Low - control

# TP
pander::pandoc.table(car::Anova(lm(TP~dosage_grp, data=newdata)))


## Drug dosage groups (low versus high) data
## -----------------------------------------------------------------------------
g1 + g2 + g3 + g4 + g6 + g7 + g8 + g9 + g10 + plot_annotation(tag_levels = 'A', title="Drug dosage effect on post-treatment blood parameters", subtitle="Comparison of nestlings receiving low dosage, high dosage or untreated control") & 
  theme(plot.tag = element_text(size = 14))
ggsave(filename = "figures/BloodDosageMultiplot.png", device = "png", dpi = 300,
       width = 6, height = 6, units = "in")
```
