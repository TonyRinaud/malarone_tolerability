...
**Changes to the dataset should be these:**

1.) Raw measures for both time points (e.g., wing_t0, wing_t1), including wing, body mass, body condition, infection status (?)  
2.) Not needed are body and ambient temperature, breathing rate ...


Experimental study comprising data from 297 nestlings that were either treated with Malarone or water. An additional subset comprises individuals that received no treatment and were therefore only exposed to repeated routine sampling and handling. 
...
malarone.data <- readr::read_delim(
  file = 'data/malarone_tolerance2.csv', delim = ",", col_types = "ffffffffnnnnffnnfninniffni") %>% 
  mutate(Date = chron(as.character(Date), format="d/m/y")) %>% 
  mutate(Diff_Inf_Class = factor(Diff_Inf_Class, levels=c("Uninfected","Increase","Stable","Decrease"))) %>% 
  mutate(Treatment_pwater = factor(Treatment_pwater, levels=c("Control","Water","Malarone"))) %>% 
  ## Scaling continuous variables
  mutate(delta_body_condition = scale((body_condition - body_condition_T0))) %>% 
  mutate(body_condition = scale(body_condition)) %>% 
  mutate(body_condition_T0 = scale(body_condition_T0)) %>% 
 # mutate(Growth_rate = scale(Growth_rate)) %>% 
  mutate(Sex = factor(
    Sex,
    levels = c(0, 1),
    labels = c("Female", "Male"))) %>% 
  filter(., !is.na(Treatment_pwater))
```


### Sanity checks:

For many variables, cases with NA values should not exist! For example:

```{r}
any(is.na(malarone.data$weight_T0))
any(is.na(malarone.data$Weight_g))
any(is.na(malarone.data$weight_T0))
any(is.na(malarone.data$Weight_g))
any(is.na(malarone.data$days_resample))

quantile(malarone.data$days_resample, na.rm = T)
```
...
g1_t1 <- ggplot(malarone.data)+
  aes(Treatment_pwater, body_condition_T0) +
  geom_hline(yintercept = mean(malarone.data$body_condition_T0, na.rm=T), linetype = "dashed") +
...
g1_t2 <- ggplot(malarone.data)+
  ...
  geom_hline(yintercept = mean(malarone.data$body_condition, na.rm=T), linetype = "dashed")+
  geom_boxplot(color ="black", outlier.shape = NA, size=.5, width=.3, na.rm = T)+
  geom_jitter(color='lightgrey', width = 0.2, height = 0, na.rm = T) +
  stat_n_text(na.rm = T, y.pos = -Inf, vjust = -1)+
  labs(x="", y="Std. body condition index")+
...
g1_delta <- ggplot(malarone.data)+
  aes(Treatment_pwater,delta_body_condition)+
...
## Run LMM
## -----------------------------------------------------------------------------
model.bci <- lmer(delta_body_condition ~ Treatment_pwater + Age_in_days + Year + (1|Nest),
                  data = malarone.data)
...
## Estimate contrasts using tukey post-hoc test
## -----------------------------------------------------------------------------
contrasts <- 
  emmeans::contrast(lsmeans::lsmeans(model.bci, "Treatment_pwater"), 'tukey') %>%
  broom::tidy()

## print contrasts
## --------------------------------------------------------------------------
pander::pandoc.table(contrasts[,c("contrast", "estimate", "std.error", "adj.p.value")])

## Predicted values for each treatment
## -----------------------------------------------------------------------------
pred.bci <- get_model_data(model.bci, type = 'pred', terms = "Treatment_pwater") %>% 
...
g1_pred_delta <- ggplot(pred.bci, aes(x = x, y = predicted)) +
  ...
  stat_n_text(data = malarone.data, aes(Treatment_pwater, body_condition), na.rm = T, y.pos = -Inf, vjust = - 1) +
  labs(x = "", y = "\U0394 std. body condition index")+
  ...
  theme(legend.position="none",
        ...
        axis.ticks.x = element_blank(),
        plot.margin = margin(0, 0, 0, 0, "cm")) +
  geom_segment(size = .8, aes(
    x = "Control", xend = "Water",
    y = pred.bci$conf.high[pred.bci$x == "Control"] + 0.02,
    yend = pred.bci$conf.high[pred.bci$x == "Control"] + 0.02)) +
  geom_text(aes(
    x = 1.5, y = pred.bci$conf.high[pred.bci$x == "Control"] + 0.03),
    vjust = 0, label = round(contrasts$adj.p.value[1], 3), size = 4.5) +
  geom_segment(size = .8, aes(
    x = "Control",
    xend = "Malarone", 
    y = pred.bci$conf.high[pred.bci$x == "Malarone"] + 0.07,
    yend = pred.bci$conf.high[pred.bci$x == "Malarone"] + 0.07)) +
  geom_text(aes(
    x = 2,
    y = pred.bci$conf.high[pred.bci$x == "Malarone"] + 0.08),
    vjust = 0, label = round(contrasts$adj.p.value[2], 3), size = 4.5) +
  geom_segment(size = .8, aes(
    x = "Water", xend = "Malarone",
    y = pred.bci$conf.high[pred.bci$x == "Malarone"] + 0.02,
    yend = pred.bci$conf.high[pred.bci$x == "Malarone"] + 0.02)) +
  geom_text(aes(
    x = 2.5, 
    y = pred.bci$conf.high[pred.bci$x == "Malarone"] + 0.03),
    vjust = 0, label = round(contrasts$adj.p.value[3], 3), size = 4.5)
```


#### Growth rate between control and malarone treated nestlings

*Note, this variable should be a daily growth rate, i.e., gain in body mass in grams per day. However measures seem to tiny for that*

Visualising the raw data

```{r} 
g2_water <- ggplot(malarone.data, aes(Treatment_pwater, Growth_rate)) +
...
lmer_gr_treat <- lmerTest::lmer(Growth_rate ~ Treatment_pwater + Age_in_days + Sex + Year + (1|Nest),
                                data = malarone.data)
...
## -----------------------------------------------------------------------------
contrasts <- 
  emmeans::contrast(lsmeans::lsmeans(lmer_gr_treat, "Treatment_pwater"), 'tukey') %>%
  broom::tidy()
...
(g2_pred <- ggplot(mod_gr, aes(x = x, y = predicted)) +
    ...
    stat_n_text(data = malarone.data, aes(Treatment_pwater, Growth_rate),
                y.pos = -Inf, vjust = -1, na.rm = T) +
    labs(x="", y="Std. growth rate")+
    geom_errorbar(aes(ymin = conf.low, ymax = conf.high), colour = 'black', size = 1.5, width = .1)+
    geom_errorbar(aes(ymin = predicted - std.error , ymax = predicted + std.error),
                  size = 8, colour = c("#1F78B4", "#33A02C", "#FF7F00"), width = 0)+
    ...
    geom_segment(size = .8, aes(
      x = "Control", xend = "Water",
      y = mod_gr$conf.high[pred.bci$x == "Control"] + 0.01,
      yend = mod_gr$conf.high[pred.bci$x == "Control"] + 0.01)) +
    geom_text(aes(
      x = 1.5, y = mod_gr$conf.high[pred.bci$x == "Control"] + 0.02),
      vjust = 0, label = round(contrasts$adj.p.value[1], 3), size = 4.5) +
    ## Control - Malarone
    geom_segment(size = .8, aes(
      x = "Control",
      xend = "Malarone", 
      y = mod_gr$conf.high[pred.bci$x == "Control"] + 0.04,
      yend = mod_gr$conf.high[pred.bci$x == "Control"] + 0.04)) +
    geom_text(aes(
      x = 2,
      y = mod_gr$conf.high[pred.bci$x == "Control"] + 0.05),
      vjust = 0, label = round(contrasts$adj.p.value[2], 3), size = 4.5) +
    geom_segment(size = .8, aes(
      x = "Water", xend = "Malarone",
      y = mod_gr$conf.high[pred.bci$x == "Malarone"] + 0.02,
      yend = mod_gr$conf.high[pred.bci$x == "Malarone"] + 0.02)) +
    geom_text(aes(
      x = 2.5, 
      y = mod_gr$conf.high[pred.bci$x == "Malarone"] + 0.03),
      vjust = 0, label = round(contrasts$adj.p.value[3], 3), size = 4.5))
```
